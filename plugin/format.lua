...
vim.api.nvim_create_user_command('FormatLock', function(opts)
  require('format').format(opts.bang, opts.args, opts.line1, opts.line2, { lock_buf = true })
end, {
  nargs = '*',
  range = '%',
  bang = true,
...
vim.api.nvim_create_user_command('FormatWriteLock', function(opts)
  require('format').format(opts.bang, opts.args, opts.line1, opts.line2, {
    hooks = {
      post = function(buf)
        vim.api.nvim_buf_call(buf, vim.cmd.write)
